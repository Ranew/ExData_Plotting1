...
# First plot
plot(data[,3],type = "l", main="", ylab="Global Active Power", xlab="" ,xaxt='n')
pts <- c(1,NROW(data)/2,NROW(data)) # Find nice points for the day labels
axis(1, at=pts, labels=c("Thu","Fri", "Sat"))

# Second plot
plot(data[,5], type="l", main="", ylab="Voltage", xlab="datetime", xaxt="n")
...
legend("topright", colnames(data)[7:9], lty=1, col=c("black", "red", "blue"), box.lty=0)
box()

# Fourth plot
plot(data[,4], type="l", ylab="Global_reactive_power", xlab="datetime", xaxt="n")
pts <- c(1,NROW(data)/2,NROW(data))
axis(1, at=pts, labels=c("Thu","Fri", "Sat"))
